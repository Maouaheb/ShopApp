...
public class GoCheckout extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		if(products.size()>0) {
			...
			gridPane.add(card, 6, products.size()+1);
			gridPane.add(paypal, 7, products.size()+1);
			boolean state=card.isSelected();
		
			if(card.isSelected()) {
				System.out.println("card is selected ");
				paymentstrategy.paycard(gridPane);
			}
			else if (paypal.isSelected()) {
				System.out.println("paypal is selected ");

				paymentstrategy.paypal(gridPane);
			}
			/*
